...
def test_experts():
    ...
    inputs = torch.randn(n_points, in_features).cuda()
    index  = torch.randint(0, n_experts, (n_points,)).cuda()

    experts = Experts(n_experts, in_features, out_features).cuda()
    out_parallel = experts(inputs, index)
    out_forloop = experts.forward_forloop(inputs, index)

    diff = torch.abs(out_forloop - out_parallel)
    print(diff.max())
...
def test_model():
    ...
    directions = torch.randn(n_points, 3)
    index  = torch.randint(0, n_experts, (n_points,))
    ...
    outputs = model(points, directions, index)
    timer.print_time('forward')
